...
load("~/SexSpecificExpression/data/allgenes_final.rda")
...
```{r}
allgenes_list <- split(x = allgenes_final, f = allgenes_final$Header)
SNPs <- split(x=SNP_list, f = SNP_list$Chromosome)
```
...
```{r, eval=FALSE}
compiled_SNP_data <- categorize_SNPs(gene_list = allgenes_list, snp_list = SNPs, chr_num = 5, bpairs = 1000)
```
